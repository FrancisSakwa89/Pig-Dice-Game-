...
async function computerTrier(franco,human){
	var roll2 = franco.roll(human);
	await sleep(2000);
	if (roll2===0){
		return roll2;
	}
	$(".rolledNum2").append("<li>"+roll2+"</li>");
	var session2 = franco.session;
	console.log("franco rolled a: "  + roll2 );
	$("#player-2-roll").text('franco rolled a: '  + roll2 );

	if (roll2 < 2) {
		var score2 = franco.score;
		playerTwoToOne();
		$("#player-2-session").text("franco's session score is: 0 " );
		$("#player-2-score").text("franco's total score is: " + score2 );

	}
	else if(session2<((franco.winscore-franco.score)/(franco.winscore-human.score))*15 && franco.sessionRolls<=3){
		$("#player-2-session").text("franco's session score is: " + session2);
		computerTrier(franco,human);
	}
	else{
		score2 = franco.hold();
		console.log("franco score: "+score2);
		await sleep(1000);
		playerTwoToOne();
		$("#player-2-score").text("franco's total score is: " + score2);
		$("#player-2-session").text("franco's session score is: "+session2 );
...
async function computerRecruit(francoSakwa,human){
	var roll2 = francoSakwa.roll(human);
	await sleep(2000);
	if (roll2===0){
		return roll2;
	}
	...
	$(".rolledNum2").append("<li>"+roll2+"</li>");
	var session2 = francoSakwa.session;
	console.log("1.Franco.Sakwa rolled a: "  + roll2 )
	$("#player-2-roll").text('Franco.Sakwa rolled a: '  + roll2 );
	if (roll2 < 2) {
		playerTwoToOne();
		var score2 = francoSakwa.score;
		$("#player-2-session").text("Franco.Sakwa's session score is: 0 " );
		$("#player-2-score").text("Franco's total score is: " + score2 );
	}
	else{
		var roll2 = francoSakwa.roll(human);
		...
		$(".rolledNum2").append("<li>"+roll2+"</li>");
		var session2 = francoSakwa.session;
		console.log("2.Franco.Sakwa rolled a: "  + roll2 )
		$("#player-2-roll").text('Franco.Sakwa rolled a: '  + roll2 );
		if (roll2 < 2) {
			playerTwoToOne();
			var score2 = francoSakwa.score;
			...
			score2 = francoSakwa.hold();
			console.log("francoSakwa score: "+score2);
			playerTwoToOne();
			$("#player-2-score").text("Franco's total score is: " +score2);
		}
	}
}
function computerGame(artificialPlayer,humanPlayer){
	if(artificialPlayer.name==="Francis Sakwa"){
		var mode="recruit";
		computerRecruit(artificialPlayer,humanPlayer);
	}
	else{
		var mode="franco";
		computerVeteran(artificialPlayer,humanPlayer);
	}
}
	...
	$("#form_players").submit(function(event){
		...
		if ($("#player_two").val()){
			...
			var level = $("input:radio[name=level]:checked").val();
      		if (level === "recruit") {
        		playerTwoName = "Francis Sakwa"
      		}
      		else if (level === "trier") {
        		playerTwoName = "Franco"
      		}
      		if (playerOneName && playerTwoName){
				var playerOne= new myPlayer(playerOneName,winscore);
				var playerTwo= new myPlayer(playerTwoName,winscore);
